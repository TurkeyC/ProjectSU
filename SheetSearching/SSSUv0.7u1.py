...
class ExcelSearchApp:
    def __init__(self, root):
        ...
        file_content_frame.pack(fill=tk.X)

        self.file_paths = []
        self.files_listbox = tk.Listbox(file_content_frame, height=5, borderwidth=1,
                                        relief="solid", highlightthickness=0)
        self.files_listbox.pack(side=tk.LEFT, padx=5, fill=tk.BOTH, expand=True)

        # Add scrollbar to listbox
        listbox_scrollbar = ttk.Scrollbar(file_content_frame, orient="vertical",
                                         command=self.files_listbox.yview)
        self.files_listbox.config(yscrollcommand=listbox_scrollbar.set)
        listbox_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        # File buttons in their own frame
        file_buttons_frame = ttk.Frame(file_frame, padding=(0, 10, 0, 0))
        file_buttons_frame.pack(fill=tk.X)

        ttk.Button(file_buttons_frame, text="浏览文件", command=self.browse_files,
                  style="Accent.TButton").pack(side=tk.LEFT, padx=2)
        ttk.Button(file_buttons_frame, text="浏览文件夹", command=self.browse_folder,
                  style="Accent.TButton").pack(side=tk.LEFT, padx=2)
        ttk.Button(file_buttons_frame, text="清除列表", command=self.clear_files).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Label(search_content_frame, text="搜索内容:").pack(side=tk.LEFT, padx=(0, 5))
        self.search_term = tk.StringVar()
        ttk.Entry(search_content_frame, textvariable=self.search_term).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=5)

        options_frame = ttk.Frame(search_frame, padding=(0, 10, 0, 0))
        options_frame.pack(fill=tk.X)

        self.case_sensitive = tk.BooleanVar()
        ttk.Checkbutton(options_frame, text="区分大小写", variable=self.case_sensitive).pack(side=tk.LEFT)

        ttk.Button(options_frame, text="搜索", command=self.search,
        ...
        status_bar = ttk.Label(root, textvariable=self.status, relief="sunken", anchor=tk.W)
        status_bar.pack(side=tk.BOTTOM, fill=tk.X)

    def configure_styles(self):
        """Configure ttk styles for modern flat look"""
        style = ttk.Style()
        ...
        try:
            ...
            pass  # Fallback to default theme if not available
        ...
        style.map("Accent.TButton",
                 background=[('active', '#005999'), ('pressed', '#004080')])
        ...
        # Make notebook tabs more modern looking
        ...
        for file_name, file_results in results.items():
            ...
            if "error" not in file_results:
                ...
                for sheet_name, df in file_results.items():
                    ...
                    text_area = scrolledtext.ScrolledText(sheet_tab, wrap=tk.WORD, font=("Consolas", 10),
                ...
                text_area = scrolledtext.ScrolledText(file_tab, wrap=tk.WORD, font=("Consolas", 10),
                                                    borderwidth=1, relief="solid")
                text_area.pack(fill=tk.BOTH, expand=True)
